...
Nun werden für beide Richtungen (wenn möglich), berechnet, weiviele 'Züge' gemacht werden müssen, um aus dem Stellpatz ausfahren zu können. Auch Kollisionen werden über einen Check in der `squash_left` und `squash_right` Funktion, und deren rekursives Verhalten mitberechnet und diese Autos passend weiter verschoben.

Zuletzt werden die beiden Richtungen aufgrund der benötigten 'Züge' verglichen. Die Variante mit der geringsten Anzahl an Zügen wird formatiert und ausgegeben.
